...
old_pattern = '''            <p className="text-slate-600 dark:text-slate-400 font-bold text-lg">?? ???? ????? ?????????</p>
...
new_pattern = '''            <p className="text-slate-600 dark:text-slate-400 font-bold text-lg">?? ???? ????? ?????????</p>
...
if old_pattern in content:
    ...
    # Don't forget to restore customerPhone in successInvoiceInfo state!
    ...
    with open('d:/TAKKA FINEL/src/components/POS.tsx', 'w', encoding='utf-8') as f:
        f.write(content)
    print("POS.tsx restored and updated successfully!")
else:
    print("Target string not found in POS.tsx.")
