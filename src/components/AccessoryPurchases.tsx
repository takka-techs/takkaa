...
import ReturnAccessoryModal from './ReturnAccessoryModal';
...
export default function AccessoryPurchases() {
  ...
  const [selectedAccessory, setSelectedAccessory] = useState<AccessoryPurchase | null>(null);
  ...
  const handlePrintInvoice = (purchase: AccessoryPurchase) => {
    const pData = {
  ...
  const fetchPurchases = async () => {
    ...
    try {
      ...
      // Only fetch accessories that were added as purchases
      const response = await fetch(`https://hoohxkrrndtfpwsrnpyr.supabase.co/rest/v1/Accessories?select=*&entry_type=eq.purchase&order=created_at.desc${qs}`, {
        headers: {
    ...
    <div className="space-y-8" dir="rtl">
      ...
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6">
        <div className="flex flex-wrap gap-4 w-full xl:w-auto order-2 xl:order-1">
          ...
          >
             ...
             <div className="text-right">
               ...
               <div className="text-[10px] font-normal opacity-70">إضافة إكسسوار بمفرده</div>
             </div>
          </button>
          ...
          <button 
             onClick={() => {
               // Open a selection modal or just the returns tab
        ...
        <div className="flex items-center gap-3 bg-slate-100 dark:bg-white/5 px-6 py-4 rounded-2xl border border-slate-200 dark:border-white/5 order-1 xl:order-2 ml-auto xl:ml-0">
          ...
          <h1 className="text-2xl font-black text-slate-900 dark:text-white">مشتريات الإكسسوارات</h1>
        </div>
      </div>
              ...
              <table className="w-full text-start border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-white/[0.03] border-b border-slate-200 dark:border-white/5">
                    ...
                    <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-start whitespace-nowrap text-center">الإكسسوار</th>
                    <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-start whitespace-nowrap text-center">الماركة</th>
                    <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-start whitespace-nowrap text-center">الفئة</th>
                    <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-start whitespace-nowrap text-center">الكمية</th>
                    <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-start whitespace-nowrap text-center">السعر</th>
                    <th className="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-start whitespace-nowrap text-center">المورد</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  {isLoading ? (
                    ...
                    <tr>
                      <td colSpan={10} className="px-6 py-20 text-center bg-white dark:bg-transparent">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <span className="text-slate-400 dark:text-slate-600 text-sm">ابدأ بإضافة إكسسوارات جديدة لتظهر هنا</span>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr key={purchase.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.01] transition-colors group bg-white dark:bg-transparent">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex flex-col items-center justify-center">
                          <div className="flex items-center gap-2">
                            ...
                            {purchase.notes && purchase.notes.includes('مرتجع') && (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button 
                            onClick={() => {
                              setBarcodeItem({ item: purchase, category: 'accessory' });
