  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (finalSupplier && Number(paidAmount) > 0 && selectedWalletId) {
        ...
        await fetch(`https://hoohxkrrndtfpwsrnpyr.supabase.co/rest/v1/treasury_transactions`, {
          ...
          body: JSON.stringify({
            ...
            description: `سداد المورد ${finalSupplier} (إكسسوار: ${formData.name})`,
            branch_id: targetBranchId,
            tenant_id: tenantId
          })
        });
      ...
      if (finalCategory && !savedCategories.includes(finalCategory)) {
        ...
        localStorage.setItem('saved_accessory_categories', JSON.stringify(updated));
            ...
            <form id="add-accessory-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  {formData.brand === 'أخرى' && (
                    <input
                      type="text" value={otherBrand} onChange={(e) => setOtherBrand(e.target.value)} required
                      placeholder="اكتب اسم الماركة..."
                      className="w-full mt-2 bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white focus:border-cyan-500 outline-none transition-all"
                    />
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  {formData.category === 'أخرى' && (
                    <input
                      type="text" value={otherCategory} onChange={(e) => setOtherCategory(e.target.value)} required
