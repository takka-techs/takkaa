...
export default function MaintenanceReport({ onBack }: { onBack?: () => void }) {
  ...
  const isTicketRefunded = (status: string | undefined | null) => {
    if (!status) return false;
    return status === 'مرتجع / تم الاسترداد' || status === 'مرتجع' || status === 'تم الاسترداد' || status.includes('مرتجع');
  };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      processDashboard(repairsData, filters, empData);
    } catch (err) {
  ...
  useEffect(() => {
    if (repairs.length > 0) {
      processDashboard(repairs, filters, employees);
    }
  }, [filters, repairs, employees]);

  const processDashboard = (data: any[], currentFilters: any, emps: any[]) => {
    let filtered = data;

    // Filter by period
    if (currentFilters.period === 'اليوم') {
      filtered = filtered.filter(r => new Date(r.created_at) >= startOfDay(new Date()));
    } else if (currentFilters.period === 'أمس') {
      filtered = filtered.filter(r => {
        const d = new Date(r.created_at);
        return d >= startOfDay(subDays(new Date(), 1)) && d < startOfDay(new Date());
      });
    ...
    filtered.forEach(r => {
      ...
      let emp = emps.find(e => e.full_name === tech);
      ...
      const isRefunded = isTicketRefunded(r.status);
      ...
      if (tMap[dStr] !== undefined && !isRefunded) {
        tMap[dStr].rev += total;
        tMap[dStr].col += paid;
      }
      ...
      techMap[tech].commissionValue += commission;
    });
    ...
    setStats({
      ...
      totalCollected: tCol,
      totalCost: tCost,
      totalCommissions: tCommission,
      refundedCount: tRefundedCount,
      netProfit: tProfit - tCommission,
    ...
    setTechData(Object.keys(techMap).map(k => ({ name: k, count: techMap[k].count, rev: techMap[k].revenue, cost: techMap[k].cost, profit: techMap[k].profit, commissionPercent: techMap[k].commissionPercent, commissionValue: techMap[k].commissionValue })));
  ...
  const handleExportExcel = () => {
    const exportData = repairs.map((r, index) => {
      ...
      const emp = employees.find(e => e.full_name === r.technician_name);
      const commissionPercent = emp ? Number(emp.maintenance_commission_value ?? emp.commission_value ?? 0) : 0;
      const commission = (profit > 0 ? profit : 0) * (commissionPercent / 100);
      ...
      const displayNetProfit = netProfit;
      ...
      return {
        ...
        'الربح الإجمالي (ج.م)': displayProfit.toFixed(2),
        'العمولة (ج.م)': displayCommission.toFixed(2),
        'صافي الربح (ج.م)': displayNetProfit.toFixed(2),
  ...
  if (filters.period === 'اليوم') {
      displayRepairs = displayRepairs.filter(r => new Date(r.created_at) >= startOfDay(new Date()));
  } else if (filters.period === 'أمس') {
      displayRepairs = displayRepairs.filter(r => {
        const d = new Date(r.created_at);
        return d >= startOfDay(subDays(new Date(), 1)) && d < startOfDay(new Date());
      });
  } else if (filters.period === 'آخر شهر') {
      const lastMonth = subDays(new Date(), 30);
    ...
    <div className="w-full text-slate-900 dark:text-white" dir="rtl">
      ...
      <PrintReportTemplate
        ...
        summary={[
          ...
          { label: 'عدد المرتجعات', value: stats.refundedCount }
        ]}
        columns={[
          ...
          { header: 'الربح الإجمالي', accessor: (item) => {
          ...
          { header: 'العمولة', accessor: (item) => {
              ...
              const profit = total > 0 ? (total - cost) : 0;
              const emp = employees.find(e => e.full_name === item.technician_name);
              const commissionPercent = emp ? Number(emp.maintenance_commission_value ?? emp.commission_value ?? 0) : 0;
              const commission = (profit > 0 ? profit : 0) * (commissionPercent / 100);
              return commission.toLocaleString();
          }, isNumeric: true },
          { header: 'صافي الربح', accessor: (item) => {
              const total = Number(item.total_amount || 0);
              let cost = 0;
              let hasParsedCost = false;
              ...
              const commissionPercent = emp ? Number(emp.maintenance_commission_value ?? emp.commission_value ?? 0) : 0;
              const commission = (profit > 0 ? profit : 0) * (commissionPercent / 100);
              return (profit - commission).toLocaleString();
          }, isNumeric: true },
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-white dark:bg-[#161b22] border border-slate-200 dark:border-white/5 rounded-xl p-5 flex flex-col items-center justify-center text-center shadow-sm relative overflow-hidden group">
          ...
          <h2 className="text-lg font-bold font-mono text-slate-900 dark:text-white">{stats.totalCommissions.toLocaleString(undefined, {minimumFractionDigits: 2})}</h2>
        </div>
        ...
        <div className="bg-white dark:bg-[#161b22] border border-slate-200 dark:border-white/5 rounded-xl p-5 flex flex-col items-center justify-center text-center shadow-sm relative overflow-hidden group">
          ...
          <h3 className="text-slate-500 dark:text-slate-400 font-medium text-[10px] mb-1">عدد المرتجعات</h3>
          <h2 className="text-lg font-bold font-mono text-slate-900 dark:text-white">{stats.refundedCount}</h2>
        </div>
               ...
               <table className="w-full text-sm text-right">
                <thead className="text-slate-500 dark:text-slate-400 font-medium text-xs">
                   <tr>
                      ...
                      <th className="px-6 py-3 text-center">العمولة</th>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-white/5 bg-white dark:bg-transparent">
                   ...
                   {techData.length === 0 && (
                      <tr><td colSpan={7} className="px-6 py-8 text-center text-slate-500">لا توجد بيانات للفنيين</td></tr>
                   )}
                </tbody>
             </table>
          </div>
             ...
             <table className="w-full text-sm text-right">
               <thead className="text-slate-500 dark:text-slate-400 font-medium text-xs">
                  <tr>
                     ...
                     <th className="px-5 py-3 font-medium text-center">الربح</th>
                     <th className="px-5 py-3 font-medium text-center">العمولة</th>
                     <th className="px-5 py-3 font-medium text-center">صافي الربح</th>
               ...
               <tbody className="divide-y divide-slate-200 dark:divide-white/5 bg-white dark:bg-transparent">
                  {displayRepairs.map((r, i) => {
                     ...
                     <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className={`px-5 py-3 text-center font-mono ${displayProfit < 0 ? 'text-rose-500 dark:text-rose-400' : 'text-emerald-500 dark:text-emerald-400'}`}>{displayProfit.toFixed(2)} <span className="text-[10px]">ج.م</span></td>
                        <td className={`px-5 py-3 text-center font-mono text-amber-500`}>{displayCommission.toFixed(2)} <span className="text-[10px]">ج.م</span></td>
                        <td className={`px-5 py-3 text-center font-mono font-bold ${displayNetProfit < 0 ? 'text-rose-500 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{displayNetProfit.toFixed(2)} <span className="text-[10px]">ج.م</span></td>
                  ...
                  {displayRepairs.length === 0 && (
                     <tr>
                        <td colSpan={14} className="px-6 py-12 text-center text-slate-500">لا توجد صيانة مطابقة</td>
                     </tr>
                  )}
               </tbody>
            </table>
