...
interface AddMultipleAccessoriesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export default function AddMultipleAccessoriesModal({ 
  isOpen, 
  onClose, 
  onSuccess,
  warehouseId
}: AddMultipleAccessoriesModalProps) {
  ...
  React.useEffect(() => {
    const stored = localStorage.getItem('saved_accessory_categories');
    if (stored) {
      try {
  ...
  const [accessoriesList, setAccessoriesList] = useState([
    { id: 1, name: '', barcode: '', quantity: '1', cost_price: '0', selling_price: '0', wholesale_price: '0', half_wholesale_price: '0', tax: '0' },
    { id: 2, name: '', barcode: '', quantity: '1', cost_price: '0', selling_price: '0', wholesale_price: '0', half_wholesale_price: '0', tax: '0' },
    { id: 3, name: '', barcode: '', quantity: '1', cost_price: '0', selling_price: '0', wholesale_price: '0', half_wholesale_price: '0', tax: '0' }
  ]);
  ...
  const handleAccessoryChange = (id: number, field: string, value: string) => {
    setAccessoriesList(prev => prev.map(acc => acc.id === id ? { ...acc, [field]: value } : acc));
  };

  const addRow = () => {
    setAccessoriesList(prev => [
      ...prev, 
      { id: Date.now(), name: '', barcode: '', quantity: '1', cost_price: '0', selling_price: '0', wholesale_price: '0', half_wholesale_price: '0', tax: '0' }
    ]);
  };
  ...
  const handleSubmit = async () => {
    // Validate
    const validAccessories = accessoriesList.filter(a => a.name.trim() !== '');
    ...
    try {
      ...
      if (finalCategory && !savedCategories.includes(finalCategory)) {
        ...
        localStorage.setItem('saved_accessory_categories', JSON.stringify(newCats));
      }

      const payload = validAccessories.map(acc => ({
        name: acc.name,
        brand: finalBrand,
        ...
        half_wholesale_price: Number(acc.half_wholesale_price) || 0,
        tax: Number(acc.tax) || 0,
        quantity: Number(acc.quantity) || 0,
        supplier: sharedData.supplier,
        entry_type: sharedData.entry_type,
        status: Number(acc.quantity) > 0 ? 'Available' : 'Out of Stock',
      ...
      if (sharedData.supplier) {
        ...
        const totalCost = validAccessories.reduce((sum, acc) => sum + ((Number(acc.cost_price) || 0) * (Number(acc.quantity) || 0)), 0);
        ...
        if (paid > 0 && selectedWalletId) {
          ...
          await fetch(`https://hoohxkrrndtfpwsrnpyr.supabase.co/rest/v1/treasury_transactions`, {
            ...
            body: JSON.stringify({
              ...
              description: `سداد للمورد (إضافة إكسسوارات متعددة)`,
              branch_id: targetBranchId,
              tenant_id: tenantId
            })
          });
  ...
  const totalCost = accessoriesList.filter(a => a.name.trim() !== '').reduce((sum, acc) => sum + ((Number(acc.cost_price) || 0) * (Number(acc.quantity) || 0)), 0);
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">إضافة إكسسوارات متعددة</h2>
            </div>
            <button 
              onClick={onClose}
                        ...
                        <tr key={acc.id} className="hover:bg-white dark:hover:bg-white/[0.01]">
                          ...
                          <td className="p-3">
                            <input 
                              type="text"
                              value={acc.barcode}
                              onChange={(e) => handleAccessoryChange(acc.id, 'barcode', e.target.value)}
                          ...
                          <td className="p-3">
                            <div className="relative">
                              <input 
                                ...
                                onChange={(e) => handleAccessoryChange(acc.id, 'cost_price', e.target.value)}
                          ...
                          <td className="p-3">
                            <div className="relative">
                              <input 
                                type="number"
                                min="0"
                                value={acc.selling_price}
                                onChange={(e) => handleAccessoryChange(acc.id, 'selling_price', e.target.value)}
                                className="w-full bg-white dark:bg-[#080c13] border border-slate-200 dark:border-white/10 rounded-lg pl-8 pr-3 py-2 text-slate-900 dark:text-white focus:border-cyan-500 outline-none text-left"
                                dir="ltr"
                              />
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 pointer-events-none">EGP</span>
                          ...
                          <td className="p-3">
                            <div className="relative">
                              <input 
                                ...
                                onChange={(e) => handleAccessoryChange(acc.id, 'wholesale_price', e.target.value)}
                                className="w-full bg-white dark:bg-[#080c13] border border-slate-200 dark:border-white/10 rounded-lg pl-8 pr-3 py-2 text-slate-900 dark:text-white focus:border-cyan-500 outline-none text-left"
                                dir="ltr"
                              />
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 pointer-events-none">EGP</span>
                          ...
                          <td className="p-3">
                            <div className="relative">
                              <input 
                                ...
                                onChange={(e) => handleAccessoryChange(acc.id, 'half_wholesale_price', e.target.value)}
                                className="w-full bg-white dark:bg-[#080c13] border border-slate-200 dark:border-white/10 rounded-lg pl-8 pr-3 py-2 text-slate-900 dark:text-white focus:border-cyan-500 outline-none text-left"
                                dir="ltr"
                              />
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 pointer-events-none">EGP</span>
                          ...
                          <td className="p-3 text-center">
                            <button 
                              ...
                              disabled={accessoriesList.length === 1}
          ...
          <div className="p-6 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] shrink-0 flex gap-3">
            ...
            >
              ...
              {isLoading ? 'جاري الحفظ...' : 'حفظ الإكسسوارات'}
            </button>
            <button 
              onClick={onClose}
              disabled={isLoading}
