...
interface ImportSparePartsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export default function ImportSparePartsModal({ isOpen, onClose, onSuccess, warehouseId }: ImportSparePartsModalProps) {
  ...
  const handleDownloadTemplate = () => {
    const templateData = [{
      'الاسم': 'شاشة iPhone 11 أصلية',
      'الفئة': 'شاشات',
      'SKU': 'IP11-SCR-ORG',
      'الباركود': '987654321012',
      'سعر التكلفة': 1200,
      'سعر البيع': 1500,
      'الكمية': 10,
      'الحد الأدنى': 2,
      'ملاحظات': 'OLED أصلية بجودة عالية'
    }];

    const worksheet = XLSX.utils.json_to_sheet(templateData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "قالب قطع الغيار");
    
    XLSX.writeFile(workbook, "spare_parts_template.xlsx");
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const partsToAdd = jsonData.map((row: any) => ({
        name: row['الاسم'] || '',
        ...
        sell_price: Number(row['سعر البيع']) || 0,
        quantity: Number(row['الكمية']) || 0,
        min_quantity: Number(row['الحد الأدنى']) || 0,
        notes: row['ملاحظات'] || '',
        user_id: userId,
        entry_type: 'import',
      ...
      const response = await fetch('https://hoohxkrrndtfpwsrnpyr.supabase.co/rest/v1/spare_parts', {
        ...
        body: JSON.stringify(partsToAdd)
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'فشل في رفع البيانات');
      }
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">استيراد قطع الغيار من Excel</h2>
            </div>
            <button 
              onClick={onClose}
          ...
          <div className="p-6 space-y-6">
              {isOwner && branches && branches.length > 0 && (
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-300">
                    <Store className="w-4 h-4 text-cyan-400" /> الفرع الذي سيتم إضافة قطع الغيار إليه
                  </label>
                  <select 
            ...
            <div 
              ...
              className="border-2 border-dashed border-slate-200 dark:border-white/10 hover:border-teal-500/50 bg-slate-50 dark:bg-white/[0.02] hover:bg-teal-500/5 rounded-2xl p-8 flex flex-col items-center justify-center text-center gap-4 cursor-pointer transition-all group"
            >
              <input 
                type="file" 
                ref={fileInputRef} 
                ...
                className="hidden" 
              />
              <div className="w-16 h-16 bg-teal-500/10 text-teal-400 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
                {isLoading ? <Loader2 className="w-8 h-8 animate-spin" /> : <Upload className="w-8 h-8" />}
              </div>
              <div>
                <p className="text-slate-900 dark:text-white font-bold mb-1">اضغط لاختيار ملف Excel</p>
          ...
          <div className="p-6 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] shrink-0 flex items-center justify-end gap-3">
            <button 
              onClick={onClose}
              className="px-6 py-2.5 rounded-xl text-sm font-bold text-slate-500 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 transition-colors"
            >
              إلغاء
            </button>
          </div>
