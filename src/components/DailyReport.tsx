...
export default function DailyReport({ onBack }: { onBack: () => void }) {
   ...
   const groupDataByType = (data: any[], typeKey: string) => {
      return data.reduce((acc, item) => {
         const type = item[typeKey] || 'other';
   ...
   const groupedSales = groupDataByType(filteredSales, 'item_type');
   const groupedReturns = groupDataByType(filteredSalesReturns, 'product_type');
   const groupedPurchases = groupDataByType(filteredPurchases, 'item_type');

   const getCategoryHeader = (type: string) => {
      switch (type) {
         case 'device': return { title: 'الأجهزة', icon: '📱', color: 'text-indigo-500' };
         case 'accessory': return { title: 'الإكسسوارات', icon: '🎧', color: 'text-purple-500' };
         case 'spare_part': return { title: 'قطع الغيار', icon: '⚙️', color: 'text-slate-500' };
         default: return { title: 'أخرى', icon: '📦', color: 'text-gray-500' };
      }
   };
                     ...
                     <div className="bg-white dark:bg-[#11151c] print:bg-white print:border-black/20 border-r-4 border-r-gray-400 rounded-2xl p-5 border-y border-l border-slate-200 dark:border-white/5 shadow-lg relative overflow-hidden group hover:border-gray-400/30 transition-colors">
                        <div className="flex justify-between items-start">
                           <div>
                              ...
                              <h4 className="text-2xl font-extrabold text-slate-900 dark:text-gray-100 print:text-black font-mono tracking-tight">{totalSalesParams.toLocaleString()} <span className="text-sm font-normal text-slate-500 font-sans">ج.م</span></h4>
                           ...
                           <table className="w-full text-sm text-right">
                              ...
                              <tbody className="divide-y divide-slate-200 dark:divide-white/5 print:divide-black/10 text-slate-700 dark:text-slate-300 print:text-slate-800 font-medium">
                                 {Object.entries(groupedSales).map(([type, items]) => {
                                    const { title, icon, color } = getCategoryHeader(type);
                                    return (
                                       <React.Fragment key={type}>
                                          {/* Category Divider Row */}
                                          <tr className="bg-slate-100/50 dark:bg-white/[0.02] print:bg-slate-100 border-y-2 border-slate-200 dark:border-white/10 print:border-black/20">
                                             <td colSpan={8} className="px-6 py-3">
                                                <div className="flex items-center gap-2">
                                                   <span className="text-xl">{icon}</span>
                                                   <span className={`font-extrabold ${color} print:text-black`}>{title}</span>
                                                   <span className="text-xs font-bold bg-white dark:bg-slate-800 px-2 py-0.5 rounded-full text-slate-500 border border-slate-200 dark:border-white/5 shadow-sm">
                                                      {items.length}
                                                   </span>
                                                </div>
                                             </td>
                        ...
                        <div className="overflow-x-auto">
                           <table className="w-full text-sm text-right">
                              ...
                              <tbody className="divide-y divide-rose-100/50 dark:divide-rose-500/5 print:divide-black/10 text-slate-700 dark:text-slate-300 print:text-slate-800 font-medium font-sans">
                                 {Object.entries(groupedReturns).map(([type, items]) => {
                                    const { title, icon } = getCategoryHeader(type);
                                    return (
                                       <React.Fragment key={type}>
                                          <tr className="bg-rose-50/50 dark:bg-rose-950/20 print:bg-slate-100 border-y-2 border-rose-100 dark:border-rose-500/10 print:border-black/20">
                                             <td colSpan={8} className="px-6 py-3">
                                                <div className="flex items-center gap-2">
                                                   <span className="text-xl">{icon}</span>
                                                   <span className="font-extrabold text-rose-600 print:text-black">{title}</span>
                                                   <span className="text-xs font-bold bg-white dark:bg-rose-900/50 px-2 py-0.5 rounded-full text-rose-500 border border-rose-200 dark:border-rose-500/20 shadow-sm">
                                                      {items.length}
                                                   </span>
                                                </div>
                                             </td>
