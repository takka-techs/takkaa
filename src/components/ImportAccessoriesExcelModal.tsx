...
import { X, Upload, FileSpreadsheet, Loader2, AlertCircle, FileText, Store } from 'lucide-react';
import * as XLSX from 'xlsx';
import { useBranch } from '../contexts/BranchContext';

interface ImportAccessoriesExcelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export default function ImportAccessoriesExcelModal({ isOpen, onClose, onSuccess, warehouseId }: ImportAccessoriesExcelModalProps) {
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "قالب الإكسسوارات");
    
    XLSX.writeFile(workbook, "accessories_template.xlsx");
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const accessoriesToAdd = jsonData.map((row: any) => ({
        barcode: row['الباركود']?.toString() || Math.floor(1000000000000 + Math.random() * 9000000000000).toString(),
        name: row['اسم الصنف'] || '',
        category: row['التصنيف'] || '',
        cost_price: Number(row['سعر الشراء']) || 0,
        selling_price: Number(row['سعر البيع']) || 0,
        quantity: Number(row['الكمية']) || 0,
        alert_quantity: Number(row['حد التنبيه']) || 5,
        supplier: row['المورد'] || '',
        status: row['الحالة'] || 'Available',
        notes: row['ملاحظات'] || '',
        user_id: userId,
        entry_type: 'import',
      ...
      const response = await fetch('https://hoohxkrrndtfpwsrnpyr.supabase.co/rest/v1/Accessories', {
        ...
        body: JSON.stringify(accessoriesToAdd)
      });
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">استيراد الإكسسوارات من Excel</h2>
            </div>
            <button 
              onClick={onClose}
              ...
              {isOwner && branches && branches.length > 0 && (
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-300">
                    <Store className="w-4 h-4 text-cyan-400" /> الفرع الذي سيتم إضافة الإكسسوارات إليه
                  </label>
                  <select 
