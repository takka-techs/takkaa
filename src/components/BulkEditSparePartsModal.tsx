...
interface BulkEditSparePartsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  selectedIds: number[];
...
export default function BulkEditSparePartsModal({ isOpen, onClose, onSuccess, selectedIds }: BulkEditSparePartsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`https://hoohxkrrndtfpwsrnpyr.supabase.co/rest/v1/spare_parts?id=in.(${selectedIds.join(',')})`, {
        method: 'PATCH',
        headers: {
          'apikey': apiKey,
          'Authorization': `Bearer ${token}`,
        ...
        >
          <div className="p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] flex items-center justify-between shrink-0">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              تعديل مجمع ({selectedIds.length} قطعة)
            </h2>
            <button 
              onClick={onClose}
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 font-medium leading-relaxed">
              قم بتفعيل الخيار بجانب الحقل الذي ترغب في تعديله وتطبيقه على جميع القطع المحددة.
            </p>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-200 dark:border-white/10">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    />
                    <label className="text-sm font-bold text-slate-700 dark:text-slate-300">اسم القطعة</label>
                  </div>
                  <input 
                    type="text" 
                    name="name"
                    ...
                    placeholder="الاسم الجديد للقطعة..."
                    className="w-full bg-white dark:bg-[#080c13] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:border-teal-500 outline-none transition-colors disabled:opacity-50"
                  />
                </div>
                ...
                <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-200 dark:border-white/10">
                  ...
                  <select 
                    ...
                    className="w-full bg-white dark:bg-[#080c13] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:border-teal-500 outline-none transition-colors appearance-none disabled:opacity-50"
                  >
          ...
          <div className="p-6 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] flex items-center gap-3 shrink-0">
            <button 
              ...
              className="flex-1 bg-teal-600 hover:bg-teal-500 text-white px-6 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
