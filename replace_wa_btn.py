...
# Add html2canvas import at the top if not exists
...
old_wa_btn_pattern = r'\{successInvoiceInfo\?\.customerPhone && \(\s*<button\s*onClick=\{.*?\}\s*className="flex-1 py-4 bg-\[#25D366\].*?</button>\s*\)'

new_wa_btn = '''{successInvoiceInfo?.customerPhone && (
                <button 
                  onClick={async () => {
                    const phone = successInvoiceInfo.customerPhone.startsWith('0') 
...
content = re.sub(old_wa_btn_pattern, new_wa_btn, content, flags=re.DOTALL)

with open('d:/TAKKA FINEL/src/components/POS.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated WhatsApp button successfully!")
