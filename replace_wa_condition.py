...
# First replace the button structure
old_btn_pattern = r'\{successInvoiceInfo\?\.customerPhone && \(\s*<button\s*onClick=\{.*?\}\s*className="flex-1 py-4 bg-\[#25D366\].*?</button>\s*\)'
...
if re.search(old_btn_pattern, content):
    ...
    with open('d:/TAKKA FINEL/src/components/POS.tsx', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Replaced button condition successfully!")
else:
