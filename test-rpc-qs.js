...
async function test(walletFormat) {
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/process_contract_bulk_payment?p_wallet_id=${walletFormat}`, {
    method: 'POST',
    headers: hdrs,
    body: JSON.stringify({
  ...
  console.log('Query param:', walletFormat, res.status, await res.text());
}
