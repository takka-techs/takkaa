...
if old_state in content:
    ...
    print("1. State type updated OK")
else:
    print("1. State type already updated or not found")

# ===== 2. Update setSuccessInvoiceInfo to include customer data =====
old_set = """      setSuccessInvoiceInfo({
        invoiceId: invoiceData.invoice_number,
        itemsCount: printCart.reduce((sum, item) => sum + item.cartQuantity, 0),
        total: total
      });"""
new_set = """      setSuccessInvoiceInfo({
        invoiceId: invoiceData.invoice_number,
        itemsCount: printCart.reduce((sum, item) => sum + item.cartQuantity, 0),
        total: total,
...
if old_set in content:
    ...
    print("2. setSuccessInvoiceInfo updated OK")
else:
    print("2. setSuccessInvoiceInfo already updated or not found")

# ===== 3. Add WhatsApp button =====
# Find the close button and add WhatsApp button before it
old_buttons = """                    <button
...
new_buttons = whatsapp_btn + """                    <button
                      onClick={() => setIsSuccessModalOpen(false)}
                      className="px-8 py-4 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 rounded-[1.25rem] font-bold text-lg transition-all"
                    >
                      إغلاق
...
if old_buttons in content:
    ...
    print("3. WhatsApp button added OK")
else:
    print("3. Close button pattern not found!")
    # Try to find it with different spacing
    import re
    pattern = r'(\s*<button\s*\n\s*onClick=\{\(\) => setIsSuccessModalOpen\(false\)\})'
    match = re.search(pattern, content)
    if match:
        print("   Found close button at position:", match.start())
    else:
        print("   Could not find close button at all")
