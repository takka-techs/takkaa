...
modal = modal.replace(/text-slate-500 hover:text-slate-900 dark:text-white/g, 'text-slate-500 hover:text-slate-800 dark:hover:text-white');
...
// Fix text-white replace in the emerald button
modal = modal.replace(/text-slate-900 dark:text-slate-950/g, 'text-emerald-950 dark:text-slate-950');
