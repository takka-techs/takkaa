-- Add tenant_id and branch_id to installment_contracts
ALTER TABLE public.installment_contracts ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES auth.users(id);
ALTER TABLE public.installment_contracts ADD COLUMN IF NOT EXISTS branch_id BIGINT REFERENCES public.branches(id);

-- Also add to installment_payments
ALTER TABLE public.installment_payments ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES auth.users(id);
ALTER TABLE public.installment_payments ADD COLUMN IF NOT EXISTS branch_id BIGINT REFERENCES public.branches(id);
...
BEGIN
    INSERT INTO public.installment_contracts (
        tenant_id, branch_id, client_id, device_id, accessory_id, spare_part_id, invoice_id, wallet_id, total_price, down_payment, 
        ...
        p_tenant_id, p_branch_id, p_client_id, p_device_id, p_accessory_id, p_spare_part_id, p_invoice_id, p_wallet_id, p_total_price, p_down_payment,
    ...
    LOOP
        INSERT INTO public.installment_payments (
            tenant_id, branch_id, contract_id, installment_no, due_amount, due_date, status
        ) VALUES (
            p_tenant_id, p_branch_id, v_contract_id, v_payment.installment_no, v_payment.due_amount, v_payment.due_date, v_payment.status
        );
