...
-- Drop ALL existing functions with this name so we don't have duplicates
DO $$
DECLARE
    rec record;
BEGIN
    FOR rec IN 
        ...
        FROM pg_proc 
        WHERE proname = 'create_installment_contract' AND pronamespace = 'public'::regnamespace
    LOOP
...
-- Create the new function with the exact needed signature
...
GRANT EXECUTE ON FUNCTION public.create_installment_contract TO authenticated;
